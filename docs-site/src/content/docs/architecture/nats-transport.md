...
This page explains what the NATS layer carries between requester and receiver: request subjects, response subjects, TCP session subjects, flow-control subjects, and the difference between Core NATS and JetStream delivery.
...
`SERVICE_ID` is embedded in request, response, session, control, and cancel subjects. NATS balances only the initial request or session-open message. After a receiver becomes the flow owner, the rest of that flow is addressed to the original requester and the selected receiver.
...
| HTTP response credit | `<request_root>.control.<receiver_service_id>.<request_id>` | requester | owner receiver control listener |
| Owner cancel | `<request_root>.cancel.<receiver_service_id>.<request_id_or_session_id>` | requester | owner receiver cancel listener |

The default receiver listen subject is `to.proxy.requests.>`. Response, session, control, and cancel listeners subscribe to service-specific wildcard scopes derived from their role and `SERVICE_ID`.
...
flowchart LR
  subgraph http_column["HTTP subjects"]
    ...
    subgraph http_credit["HTTP response credit"]
      direction LR
      requester_http_credit["requester"] -->|response_credit| ctrl_subject["to.proxy.control.rec-1.*"]
      ctrl_subject --> receiver_http_credit["receiver"]
    end
  end
...
- response, session, control, and cancel listeners use plain wildcard subscriptions;
...
- request, response, session, control, and cancel listeners are pull consumers on `NATS_STREAM`;
...
| Receiver HTTP response credit | `<NATS_CONSUMER_NAME>-control-<SERVICE_ID>` |
...
For a multi-instance receiver pool, every live instance must have a unique `SERVICE_ID`. Core NATS receiver replicas that share work should use the same `LISTEN_SUBJECT` and `NATS_QUEUE_GROUP`; JetStream receiver replicas should share the same base `NATS_CONSUMER_NAME` for request work. Do not split Core and JetStream by message plane: `NATS_MODE` applies to request, response, session, control, and cancel traffic together.
